...
run_cell() {
	...
	[[ "$in_format" == BGR ]] && { input_colorimetry=",colorimetry=sRGB"; parser_colorimetry="colorimetry=sRGB"; }
	...
	if ! d5_gst_ssh "timeout 60 gst-launch-1.0 -e filesrc location='$remote_input' ! rawvideoparse format=$parse_format width=$SRC_WIDTH height=$SRC_HEIGHT framerate=1/1 $parser_colorimetry ! video/x-raw,format=$in_format,width=$SRC_WIDTH,height=$SRC_HEIGHT$input_colorimetry ! $reference_filter ! video/x-raw,format=$out_format,width=$out_w,height=$out_h,colorimetry=$output_colorimetry ! filesink location='$remote_scratch/$label.ref.raw'" \
		>"$ref_log" 2>&1; then
		record_cell "$operation" "$in_format" "$out_format" "$out_w" "$out_h" \
