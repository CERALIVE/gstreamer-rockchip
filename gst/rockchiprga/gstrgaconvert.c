...
static gboolean
gst_rga_convert_aligned_info (const GstVideoInfo * input, GstVideoInfo * aligned,
    ...
    const GstVideoInfo * info, guint * size)
{
  ...
  if (!allocator)
    return NULL;
  if (!gst_rga_convert_aligned_info (info, &aligned, &alignment)) {
    gst_object_unref (allocator);
    return NULL;
  }
  ...
  *size = GST_VIDEO_INFO_SIZE (&aligned);
  ...
  if (!gst_buffer_pool_set_config (pool, config)) {
    GST_ERROR_OBJECT (self, "failed to configure DMA-BUF pool for %"
        GST_PTR_FORMAT, caps);
    ...
    GstQuery * decide_query, GstQuery * query)
{
  ...
  if (!caps || !gst_rga_convert_info_from_caps (caps, &info, &memory) ||
      !gst_rga_convert_aligned_info (&info, &aligned, &alignment))
    return FALSE;
...
static gboolean
gst_rga_convert_pool_uses_dmabuf (GstBufferPool * pool)
{
  GstStructure *config;
    ...
    GstQuery * query)
{
  ...
  for (i = 0; i < gst_query_get_n_allocation_pools (query); i++) {
    ...
    if (gst_rga_convert_pool_uses_dmabuf (pool)) {
      selected = i;
      break;
    }
  }
